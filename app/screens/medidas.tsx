  ...
  const fetchMedidas = async () => {
    ...
    try {
      const res = await axios.get(
        `https://adamix.net/medioambiente/medidas?categoria=${encodeURIComponent(categoria)}`
      );
      setMedidas(res.data);
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          !loading ? <Text style={{ color: '#1B5E20' }}>Ingresa una categoría y presiona Buscar.</Text> : null
        }
      />
    </View>
  );
